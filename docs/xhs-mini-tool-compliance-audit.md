...
- 新增 `npm run build:xhs`：白名单生成 `dist/xhs/`。~~当前产物为 86 个受支持文件、28.3 MiB；40 MiB 是项目自设回归预算~~（2026-08-21 更新：已按官方 `zip-artifact-spec` §6 的**单体 zip 10MB 硬上限**整改——当前产物 136 个文件、9.6 MiB（zip 实测 9.24MB），构建脚本预算口径 9.6 MiB，超限即报错）。自定义输出目录若已存在会拒绝覆盖，避免递归误删。
- 运行图全面转 webp：24 张卡面 896×1344 q75（母版单次 Lanczos + 轻锐化，合计 5.9MB）、24 张高度图 512×768 灰度（0.66MB）、24 张图鉴缩略图 320×480 q68（0.85MB）；地形皮肤转 webp q78（520KB，manifest 已记录派生关系）。
- 印面小篆恢复：崇羲篆体 CC-BY-ND 禁止子集化，改为构建期预渲染 50 枚白字透明底竖排 PNG（`production/build-seal-pngs.py` → `assets/seals/`，344KB），`card-art.js` 贴图绘制并修复了 3 字以上称号印只画首字的旧缺陷；缺图回退 MaShanZheng 字体排版。
...
- 投影 SVG 不再内嵌 `data:` JPEG；由 SRTM Plus 高程派生的 Natural Earth Shaded Relief 经同投影重采样、程序化青绿设色后，与矢量覆盖层作为包内本地图片分层加载。运行包不含原始 TIFF、下载器、Python、WASM 或 Worker，同时保留 Natural Earth 陆地裁切与中国轮廓。
...
- `npm run check` 当前为地图 14 项 + 小红书 8 项，共 22 项通过；模拟 `window.xhs.miniTool` 的浏览器运行验证确认 `writeTempFile → postNote/saveImageToPhotosAlbum` 的实际调用顺序和参数。
