...
**当前结论：代码、离线包和移动 Web 运行验证均已通过；提交前仍需在小红书 9.37+ 的 iOS / Android 真机容器完成最终验收。**

- 新增 `npm run build:xhs`：白名单生成 `dist/xhs/`，产物为 62 个受支持文件、唯一 `index.html`、28.2 MiB；40 MiB 是项目自设回归预算，不冒充平台包体上限。
- 新增桥接契约与包合规测试：`postNote` 使用 `mediaInfo.image_resources`，大图先经 `writeTempFile`，相册接口只传 `filePath`；自动拦截外链、内联脚本、禁用 API、不支持扩展名和多 HTML。
- 24 张正图与 24 张高度图从 548.9 MiB 的 4K 母版改为约 25 MiB 的运行图；首屏不再全量预载，抽中、详情和图鉴缩略卡进入视口时才加载对应人物。
- 移除 `<a download>` 降级和 21 MiB OTF，引入包内 WOFF2；修复全幅人物无意义的 `.webp/.png` 探测，并把地图和详情操作目标扩大到 44×44 px。
- 390×844 的独立产物实测：首屏 0 控制台报错，只加载入口、7 个脚本、CSS、字体和图标；请卡后只新增该人物的正图与高度图，地图打开后只新增投影 SVG。请卡、图鉴、详情、地图点位故事均可操作。
- `npm run check` 当前为地图 10 项 + 小红书 4 项，共 14 项通过；模拟 `window.xhs.miniTool` 的浏览器运行验证确认 `writeTempFile → postNote/saveImageToPhotosAlbum` 的实际调用顺序和参数。

以下 Findings 保留为整改前基线，便于追踪问题来源；其 C1–C3、H1–H4、M1、M3 已在当前实现中关闭。M2 的 9.37+ 客户端要求与双端真机验证仍是发布门禁。
