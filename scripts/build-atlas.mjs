...
const [land, rivers, lakes, countries] = await Promise.all([
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" role="img"
  ...
  data-skin="${AtlasModel.asset.skinStyle}" data-skin-source="${path.basename(skinPath)}"
  data-terrain-source="${AtlasModel.asset.terrainSource}"
  data-boundary-source="natural-earth-admin-0-110m">
  <title id="atlas-title">华夏人物行旅地理底图</title>
  <desc id="atlas-desc">Natural Earth 地理轮廓经 Web Mercator 投影生成，叠加由 SRTM Plus 高程派生的青绿地形与中国大陆轮廓。</desc>
  <metadata>Geographic source: Natural Earth public domain; terrain: Natural Earth Shaded Relief derived from SRTM Plus; style: natural-earth-srtm-qinglu-v2; generated by scripts/build-atlas.mjs.</metadata>
  <defs>
  ...
  <g clip-path="url(#frame)">
    <path id="ocean-mask" d="M0,0H${width}V${height}H0Z${landPath}" fill="url(#sea)" fill-rule="evenodd"/>
    <path d="${graticulePath()}" fill="none" stroke="#ecd990" stroke-opacity=".10" stroke-width="1.5"/>
    <path d="${landPath}" transform="translate(0 13)" fill="#594b2d" fill-rule="evenodd" opacity=".06"/>
    <path d="${landPath}" fill="url(#land)" fill-opacity=".04" fill-rule="evenodd" filter="url(#land-shadow)"/>
