...
function mountainGlyph(longitude, latitude, scale) {
  const point = project([longitude, latitude]);
  const s = scale;
  return `<g transform="translate(${n(point.x)} ${n(point.y)})" opacity=".52">
    <path d="M${-42 * s},${18 * s} L${-12 * s},${-25 * s} L${5 * s},${1 * s} L${24 * s},${-33 * s} L${54 * s},${18 * s}Z" fill="#225c58" stroke="#d0ae5d" stroke-width="${2.2 * s}"/>
    <path d="M${-12 * s},${-25 * s} L${-3 * s},${-10 * s} L${5 * s},${1 * s} M${24 * s},${-33 * s} L${31 * s},${-14 * s}" fill="none" stroke="#f0d88f" stroke-width="${1.4 * s}"/>
  </g>`;
}

const [land, rivers, lakes] = await Promise.all([
...
const mountains = [
  [76, 35, 1.2], [82, 32, 1.1], [88, 30, 1], [94, 29, 1.15],
  [101, 29, 1], [106, 34, .8], [110, 43, .9], [118, 42, .7]
].map((item) => mountainGlyph(...item)).join('');
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" role="img"
  ...
  <desc id="atlas-desc">Natural Earth 地理轮廓经 Web Mercator 投影生成，叠加青绿山水装饰。</desc>
  <metadata>Geographic source: Natural Earth public domain; generated by scripts/build-atlas.mjs.</metadata>
  <defs>
    <linearGradient id="sea" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#173f4a"/><stop offset="1" stop-color="#0d303b"/></linearGradient>
    <linearGradient id="land" x1="0" y1="0" x2=".7" y2="1"><stop stop-color="#d9c477"/><stop offset=".52" stop-color="#90aa72"/><stop offset="1" stop-color="#3f8170"/></linearGradient>
  ...
  <g clip-path="url(#frame)">
    ...
    <path d="${landPath}" fill="url(#land)" fill-rule="evenodd" stroke="#e0bd65" stroke-width="4" stroke-linejoin="round" filter="url(#land-shadow)"/>
    ...
    <path d="M0 75 Q320 12 640 70 T1280 65 T1920 80 T2560 62" fill="none" stroke="#f8e9bf" stroke-opacity=".18" stroke-width="28"/>
  </g>
</svg>
`;
